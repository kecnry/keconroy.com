...
export class Header extends React.Component {
  ...
  render() {
    ...
    let height1 = `${(0.8-scrollPerc) * this.state.windowHeight}px`
    let y = `-${0.4 * this.state.windowHeight + scrollPerc * this.state.windowHeight}px`
    ...
    console.log("scrollPerc" + scrollPerc)
    ...
    return (
      <div>
        <svg width="100%" height="100%" style={{position: 'fixed'}}>
          ...
          <rect x={x5} y={y} width="25%" height="120%" style={{fill:blue1, transform:transform5}} className="headerRectangle" />
        </svg>
      </div>
    )
  }
